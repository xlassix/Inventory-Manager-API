...
export const editPurchaseorderService = async (data) => {
  ...
  const db_data = await PurchaseOrder.findById(data.id)
    .populate('related_items')
    .lean()
    .exec()
  ...
  assert.equal(
    differenceBy(data['related_items'], db_data['related_items'], '_id').length,
    0,
    'item are not present in this order'
  )
  data['related_items'] = unionBy(
    db_data['related_items'],
    data['related_items'],
    '_id'
  )
  return await PurchaseOrder.findByIdAndUpdate(data.id, data, { new: true })
    .lean()
    .exec()
}
